...
function Section({
  ...
  return (
    <div className="border-b border-[#00243f]">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center justify-between w-full text-left px-5 py-3.5 hover:bg-[#002237] transition-colors"
      >
        <span className="text-[10px] font-bold text-[#4d8ab5] uppercase tracking-widest">{title}</span>
        <svg
          className={`w-3 h-3 text-[#336b8a] transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
...
export function FilterPanel({ filterGroups, filters, onChange, onReset, activeFilterCount }: FilterPanelProps) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#00243f]">
        <span className="text-[10px] font-bold text-[#4d8ab5] uppercase tracking-widest flex items-center gap-2">
          Filters
          {activeFilterCount > 0 && (
            <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-[#005596] text-white text-[10px] font-bold">
              {activeFilterCount}
            </span>
          )}
        </span>
        {activeFilterCount > 0 && (
          <button
            onClick={onReset}
            className="text-xs text-[#ffc425] hover:text-yellow-300 font-semibold transition-colors"
          >
            Reset all
          </button>
        )}
